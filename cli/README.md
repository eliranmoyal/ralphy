...
ralphy --jira-ticket PROJ-123 --jira-subtasks # run all subtasks of the parent
```

Requires `JIRA_HOST`, `JIRA_EMAIL`, and `JIRA_TOKEN` (Atlassian API token) environment variables. Config can override host/email via `.ralphy/config.yaml`. Fetches description and comments; transitions to "Done" on completion (or custom `toTransition` from config). With `--jira-subtasks`, loops through incomplete subtasks of the parent.
...
Save prompts and AI responses to `.ralphy/logs/` for debugging or auditing:
...
Each task creates a markdown file with the full prompt and AI response. Files are named `{timestamp}-{task-slug}.md`.
